...
const vec3 CORP_BLACK = vec3(0.0, 0.0, 0.0);
const vec3 CORP_WHITE = vec3(1.0, 1.0, 1.0);
const vec3 CORP_RED   = vec3(213.0 / 255.0, 0.0, 55.0 / 255.0);
...
vec3 getCycleColor(float t) {
  float offset = t * 2.5;
  float phase = mod(iTime * colorCycleSpeed + offset, 3.0);
  if (phase < 1.0) {
    return mix(CORP_BLACK, CORP_WHITE, smoothstep(0.55, 1.0, phase)) * 0.9;
  } else if (phase < 2.0) {
    return mix(CORP_WHITE, CORP_RED, smoothstep(0.55, 1.0, phase - 1.0)) * 0.9;
  } else {
    return mix(CORP_RED, CORP_BLACK, smoothstep(0.55, 1.0, phase - 2.0)) * 0.9;
  }
}

vec3 getLineColor(float t, vec3 baseColor) {
  if (colorCycleSpeed > 0.0) {
    return getCycleColor(t);
  }
